...
4. **Error mapping.** Implementations map driver errors via `mapDatabaseError`; unique conflicts are detected with `executor.isUniqueConflict(err)` and translated to the documented domain error (e.g. 409 `errorHandler.databaseConflict`) — no sqlite pre-check vs PG constraint divergence.
