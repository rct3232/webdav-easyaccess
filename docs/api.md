...
| GET    | `/api/files/trash`                        | Token          | List trashed nodes visible to the caller (write-permission survivors + admin). Query: `limit` (default 50), `offset`. Returns `{ items: [{ nodeId, name, type, deletedAt, displayPath, hasReadPermission, hasWritePermission, hasAdminPermission }], total }`.                                            |
