...
| E2E-ADMINCFG-009 | Admin config | T0 keys (incl. `JWT_SECRET`, `WEA_DB_PASSWORD`) render read-only in Section B, masked, in registry order — supersedes the D5 "metadata group absent" assertion | covered | `e2e/admin-config.spec.ts` | Section split (W-B, 2026-09): T0 keys are shown in the Deploy-time summary, not hidden |
