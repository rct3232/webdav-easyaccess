...
import { Menu, MenuItem, ListItemText, Box, Chip } from '@mui/material';
...
const UserSelectionMenu = ({
  ...
  const { displayUsers, availableUsers, currentIsUserBaseFolder, reviewRequesterOption } =
        ...
        <>
          ...
          {!reviewRequesterOption.alreadyAdded ? (
            <MenuItem
              onClick={(e) => {
                e.stopPropagation();
                handleUserSelect(reviewRequesterOption.userId, requesterName);
              }}
            >
              <ListItemText primary={requesterName} secondary={t('dialogs.applicant')} />
            </MenuItem>
          ) : (
            <MenuItem disabled>
      ...
      <>
        ...
        {availableUsers.length === 0 ? (
          ...
          availableUsers.map((targetUser) => (
            ...
            >
              <ListItemText primary={targetUser.username} secondary={targetUser.email} />
            </MenuItem>
          ))
        )}
      </>
