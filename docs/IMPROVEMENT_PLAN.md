...
| -- | ------ | ---- | ------------------------------------- |
| DEF-12 | DEFERRED (R1+R3 landed 2026-09-09) | S3 **overwrite** failure now rolls back to the pre-state — node back to `active`, previous active object_map row reactivated, pending v_{k+1} row + blob deleted, file re-downloadable as the previous version (`uploadService.md` §2.3/§2.5). The GC is retention-category aware with a **last-good guard** (an orphaned row on a stuck `pending_upload` node with no active row is never deleted — `gcService.md` §2) and cleans stale pending-live rows after `GC_PENDING_STALE_DAYS` (default 3d, 0=off). Remaining: R2 `pending_upload` scan/repair + startup report (DEF-13), WebDAV `overwrite` `orphaned_node` recovery incl. D5a (`retry-delete` deletes the remote blob) + D5d (`force-active` remote-existence check). | `docs/spec/server/services/uploadService.md` §2.5, `docs/spec/server/services/gcService.md` §2, `docs/spec/server/services/fileService.md` §4, `docs/features/core-service-layer.md` |
| DEF-13 | DEFERRED (partially covered 2026-09-09) | Process death between an upload's TX1 commit and the blob write leaves orphaned `pending_upload` rows. Now bounded: the GC last-good guard protects the previous version's row/blob, stale pending rows+blobs are cleaned after `GC_PENDING_STALE_DAYS`, and Tier 2 keeps pending-live keys. Remaining: startup stale-report (report-only) + admin scan/repair actions (R2). | `docs/spec/server/services/uploadService.md` §2.5 |
