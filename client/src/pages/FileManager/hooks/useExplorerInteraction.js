...
export function useExplorerInteraction({
  ...
  const handleFileClickInternal = useCallback(
    async (file, options = {}) => {
      if (!file) return;

      const { forceOpen = false } = options;
      const inSelectionMode = forceOpen ? false : selectionMode;
      const { trackRecentFileClick, clearTracking, handleRecentFileError, setRecentFileToPreview } =
        recentFileApi || {};
  ...
  const handleMoreClick = useCallback(
    (file, event) => {
      ...
      if (isMobile) {
        ...
        setContextMenu(
          event ? { mouseX: event.clientX, mouseY: event.clientY } : { mouseX: 0, mouseY: 0 }
        );
