...
and the optional JWT secret (with its boot-time ephemeral fallback).
...
- **Persistence target:** non-T0 settings are upserted into the connected metadata DB
  `settings` table (`POST /api/setup/apply`); **T0 keys are never written to the DB by the
  wizard** — they are `.env`-owned (decisions **D5/D6/D7**, Phase B). Of the T0 keys only
  `JWT_SECRET` is `.env`-writable, and only when the operator supplies one (an omitted `jwt`
  block leaves `.env` untouched; the boot-time ephemeral fallback then applies). The metadata
  backend is the default SQLite unless the remote DB keys (`WEA_DB_*`) are declared in `.env` —
  see `docs/SETUP.md`.
...
- **Admin account:** username is fixed to `admin` (matches `ensureDefaultAdmin`); the wizard
  ...
  ephemeral random secret applies.
...
1. **Boot layer → `.env`:** `apply` writes `JWT_SECRET` into the _resolved active env file_
   (the same path the loader used — `server/index.js:10-18`; `DOTENV_CONFIG_PATH` or
   `<root>/.env`) **only when the operator supplies one** — the `jwt` payload block is optional
   and no other key is generated or written. An omitted `JWT_SECRET` leaves `.env` untouched and
   the boot-time ephemeral random secret applies. A restart is required for a written boot-frozen
   value to take effect. The metadata connection (the remote `WEA_DB_*` block /
...
The two layers are independent: the wizard/CLI write the optional `JWT_SECRET` into `.env`
(only when the operator supplies one) plus the DB `settings` rows, while the existing admin
settings routes never write `.env`.
...
The wizard sets the admin **password** (username fixed to `admin`); `JWT_SECRET` is optional —
apply writes it to `.env` only when the operator supplies one (otherwise the boot-time ephemeral
random secret applies). The wizard does not create or rename users. Apply never writes an
`ADMIN_DEFAULT_PASSWORD` value and never
...
## Optional JWT secret and boot-time ephemeral fallback (D7)

`JWT_SECRET` remains a T0, `.env`-owned key (frozen at require time, read-only/masked in the
admin Deploy-time summary, rejected by admin config `PUT`, excluded from config-sync) but is now
**optional** — it is never a completeness condition and never a boot blocker:

- When `JWT_SECRET` is **unset or empty**, `server/utils/auth.js` generates an **ephemeral random
  secret at boot for that process**. A restart yields a new secret and invalidates all sessions —
  a full re-login, because refresh tokens are in-memory. A single-instance deployment may rely on
  this fallback.
- When set (any value, the legacy placeholder included) it is used as-is; the well-known
  placeholder (`your-secret-key-change-in-production`) only triggers a "change it" warning, never
  an error. The production fail-fast throw (`JWT_SECRET must be set in production`) and the
  `[setup-mode]` boot detour for a default/missing secret are removed.
- **Multi-instance deployments must set one unified `JWT_SECRET`** across all instances, otherwise
  tokens signed by one instance are rejected by another (with no env value, each instance signs
  with its own ephemeral secret).
- Setting a value persists it: wizard/CLI `apply` writes `JWT_SECRET` to `.env` when the operator
  supplies one, so sessions survive restarts. When no secret is supplied, `.env` is left untouched
  and the boot-time ephemeral fallback applies.
...
- Fresh production boot (`NODE_ENV=production`, no env) reaches `/api/setup/status` instead of
  crashing; a complete-but-JWT-missing production install boots normally, signing tokens with an
  ephemeral random secret (a restart invalidates those sessions).
