...
/**
 * sqlite implementation of PermissionRepository (`?` placeholders).
 * @param {import('../../../../infrastructure/db/executor').DbExecutor} executor
 */
module.exports = function createSqlitePermissionRepository(executor) {
  return {
    dialect: 'sqlite',
    ...
    async deleteSharePermission(token) {
      try {
        await executor.transaction(async (tx) => {
          await tx.run('DELETE FROM permissions_shares WHERE token = ?', [String(token)]);
        });
      } catch (error) {
        throw mapDatabaseError(error);
      }
    ...
    async deleteAllUserPermissions(userId) {
      try {
        await executor.transaction(async (tx) => {
          await tx.run('DELETE FROM permissions_user_paths WHERE user_id = ?', [Number(userId)]);
          await tx.run('DELETE FROM permissions_user_files WHERE user_id = ?', [Number(userId)]);
        });
      } catch (error) {
        throw mapDatabaseError(error);
      }
    ...
    async listSharedWithUser(userId, homeRootNodeId) {
      ...
      try {
        const [pathRes, fileRes] = await Promise.all([
          executor.query(buildSharedSql('permissions_user_paths', '?', '?', excludeOwn), params),
          executor.query(buildSharedSql('permissions_user_files', '?', '?', excludeOwn), params),
        ]);
        const seen = new Set();
        const result = [];
        for (const row of [
    ...
    async deleteUserSubtreePermissions(userId, rootNodeId) {
      ...
      try {
        const pathRes = await executor.run(
          buildSubtreeRemovalSql('permissions_user_paths', '?', '?'),
          [uid, root]
        );
        const fileRes = await executor.run(
          buildSubtreeRemovalSql('permissions_user_files', '?', '?'),
          [uid, root]
        );
        return { removedPaths: pathRes.changes || 0, removedFiles: fileRes.changes || 0 };
      } catch (error) {
