...
>
> **Remaining notes for the user:** (1) **Admin root = filesystem root `/`** (user-confirmed): production never creates an admin home node (`cleanupService` skips admins), `useFolderTreeController`/`useFileManager` treat admin home as null, and `folders/create` + upload now accept `parentNodeId: null` for admin — so admin browses all users' home directories from `/`, while regular users (e.g. `user1`) have named home nodes `/user1_x` with "Home › user1_x › …" breadcrumbs and a username-labeled tree root. The E2E seed no longer creates a `/admin` node and the core-flow specs assert root-relative paths (`/flow-folder`). `display_path` remains absolute (includes the owning user's home node name) for non-root nodes. (2) Benign `GET /api/permissions/check?nodeId=null → 400` log noise during share-public flows (client sends an unresolved nodeId; the server answers 400 but via a DB `22P02` cast rather than a clean validation — no test impact) — candidate follow-up. (3) **`E2E_LATER_WAVES=1` fully validated (2026-08-07): full suite green in both modes** — s3 144 passed / 18 skipped / 0 failed; webdav 128 passed / 34 skipped / 0 failed (skips = s3-only gating + mobile/desktop-only mypage variants). Three E2E robustness fixes landed to get there: (a) `E2E-OVERLAY-008` `__recent__` assertion fixed to wait for the opened file's listing entry (recent tracking fires on file open, not create); (b) mobile long-press helpers (`longPressItem`, `mobileLongPressFile`) retry the press until the `bulk-action-move` toolbar appears (was intermittently failing to enter selection mode); (c) **per-project data isolation**: `e2e/00-project-setup.spec.ts` re-runs the seed (TRUNCATE + re-seed) at the start of each project so the mobile project does not inherit the desktop project's accumulated admin-root state — the app's 50-item infinite-scroll cap + folders-first sort made late-uploaded files invisible once the root exceeded 50 folders.
