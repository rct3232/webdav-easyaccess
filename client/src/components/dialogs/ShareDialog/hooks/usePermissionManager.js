...
export const usePermissionManager = (_config) => {
  ...
  const handleAddUserPermission = useCallback(
    (nodeId, targetUserId, permission, subfolderNodeIds = []) => {
    ...
    setFolderPermissions((prev) => {
      ...
      const togglePerm = (targetNodeId, forcePermission = null) => {
        ...
        if (userPermMap) {
          ...
          const newPermission =
            forcePermission ||
            (currentPermission === PERMISSIONS.READ ? PERMISSIONS.WRITE : PERMISSIONS.READ);
  ...
  const hasPermissionChanged = useCallback(
    (nodeId) => {
