...
export function useExplorerProgress({
  ...
  const dismissProgressItem = useCallback((progressId) => {
    if (!progressId) return;
    updateProgress({ id: progressId, remove: true });
  }, [updateProgress]);

  // Cancel upload: single file
  const cancelUploadFile = useCallback((progressId, fileName) => {
  ...
  const cancelAllUpload = useCallback((progressId) => {
    if (!progressId?.startsWith('upload_drop_')) return;
  ...
  const cancelAllProgress = useCallback((progressId) => {
  ...
  const retryProgress = useCallback(async (progressId) => {
    const progressItem = progressItems.find(item => item.id === progressId);
