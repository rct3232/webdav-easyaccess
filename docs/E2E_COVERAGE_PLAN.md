...
- `e2e/global-setup.ts` never tears the stack down (that would wipe the Postgres volume out from under the running server). It resets the data state instead: TRUNCATE all app tables (preserving `_schema_migrations`) + re-seed admin and base users with home `file_nodes` roots. It ensures the MinIO bucket in both modes (the webdav-mode app and the migration E2E target it too) and empties it in s3 mode only, for a deterministic baseline. `e2e/global-teardown.ts` empties the bucket (s3 mode) and stops the stack (`down -v`).
- **Setup-wizard runtime assumption (hermetic):** the setup spec is fully isolated from the shared E2E state above. It spawns its own scratch server instance on `:5003` (own env file via `DOTENV_CONFIG_PATH`, own sqlite path, own scratch PG DB) and supervises its own process lifecycle because restart is the behavior under test. The shared `:5002` server and `:3000` client still boot for the run but are unused by the setup projects.
...
- `E2E-SETUP-001` through `E2E-SETUP-004` are now covered in `e2e/setup-wizard.spec.ts`, running in the dedicated `setup-wizard-desktop`/`setup-wizard-mobile` projects. The spec is fully hermetic: each case spawns its own scratch server on `:5003` (own env file, own sqlite path, own scratch PG DB `webdav_e2e_setup`), supervises its own restart, and cleans up in `afterEach`. The wizard projects exist only in the s3 full-mode matrix — `playwright.config.ts` defines them in its non-webdav branch — so the suite never runs under `E2E_BACKEND_MODE=webdav` (the webdav smoke defines only `webdav-smoke-setup`/`webdav-smoke-desktop` and does not include the setup-wizard suite). `E2E-SETUP-005` is the per-project DB reset infrastructure test in `00-project-setup.spec.ts`.
...
| E2E-EXP-012 | Explorer | Rename a file keeps its content byte-identical              | user1 | `/files`        | both     | uploaded file exists in a folder      | renamed file resolves at the new path, old path is gone, download bytes equal the original fixture                               | P0       | playwright        | `e2e/core-flow.shared.spec.ts`  | covered | Ported from E2E-S3PG-002; single-node rename API + content integrity; backend-agnostic (regression net for WebDAV re-upload sync) |
| E2E-EXP-013 | Explorer | Move a file across folders keeps its content byte-identical | user1 | `/files`        | both     | source and destination folders exist  | file resolves at the new path, old path is gone, download bytes equal the original fixture, listing shows it only in destination | P0       | playwright        | `e2e/core-flow.shared.spec.ts`  | covered | Ported from E2E-S3PG-003; single-node move API + content integrity; backend-agnostic                                              |
| E2E-EXP-014 | Explorer | Standard-user home shows Home labels in breadcrumb and folder tree | approved user | `/files` | both | seeded standard user owns a folder inside own home | inside an own-home folder: breadcrumb first chip is Home and the username never renders as a chip; folder-tree home row is Home (username absent) | P1 | playwright | `e2e/core-flow.shared.spec.ts` | covered | Regression net for the home-root label fix (RCA Case C, 2026-09-08); follows the E2E-EXP-012/013 standard-user login pattern |
...
| ID            | Domain       | Flow                                                             | Role      | Entry route | Viewport | Preconditions                                                            | Expected outcome                                                                                                                                                                                          | Priority | Recommended layer | Planned spec file          | Status  | Notes                                                                                      |
| ------------- | ------------ | ---------------------------------------------------------------- | --------- | ----------- | -------- | ------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------- | ----------------- | -------------------------- | ------- | ------------------------------------------------------------------------------------------ |
| E2E-SETUP-001 | Setup wizard | First-run sqlite+webdav configure-and-restart                    | anonymous | `/setup`    | both     | fresh scratch instance on `:5003` (harness pre-provisions the scratch `.env`)                                   | apply re-writes the pre-provisioned scratch `.env` (its only `.env` write is the supplied `JWT_SECRET`); after restart login with the wizard-chosen admin works, upload/download round-trip succeeds, and `/setup` redirects to `/login`                                         | P0       | playwright        | `e2e/setup-wizard.spec.ts` | covered | Case 1; wizard suite runs only in the s3-mode matrix (setup-wizard-desktop/-mobile)         |
| E2E-SETUP-002 | Setup wizard | First-run sqlite+s3 configure-and-restart writes `S3_*` keys     | anonymous | `/setup`    | both     | fresh scratch instance on `:5003` (harness pre-provisions the scratch `.env`), MinIO `:9010`                    | the `S3_*`/`AWS_*` keys land in the scratch DB `settings` table (not `.env`); restart lands in a configured app                                                                                                               | P0       | playwright        | `e2e/setup-wizard.spec.ts` | covered | Case 2 (s3 file backend via scratch MinIO `:9010`); wizard suite never runs under `E2E_BACKEND_MODE=webdav` |
| E2E-SETUP-003 | Setup wizard | First-run postgresql+webdav seeds scratch PG, not scratch sqlite | anonymous | `/setup`    | both     | fresh scratch instance on `:5003` (harness pre-provisions the scratch `.env`), scratch PG DB `webdav_e2e_setup` | after restart `_schema_migrations` + users exist in the scratch PG DB; the scratch sqlite file holds only the boot-time default admin (the wizard's postgresql apply never seeds/updates users in sqlite) | P0       | playwright        | `e2e/setup-wizard.spec.ts` | covered | Case 3; wizard suite runs only in the s3-mode matrix                                       |
| E2E-SETUP-004 | Setup wizard | Complete-state gate locks wizard and admin writes                | anonymous | `/setup`    | both     | completed setup (post-restart)                                           | `POST /api/setup/apply` → 403; `/setup` redirects to `/login`; file APIs work                                                                                                                             | P0       | playwright        | `e2e/setup-wizard.spec.ts` | covered | Case 4 (security); wizard suite runs only in the s3-mode matrix                             |
...
| ----------- | --------- | ----------------------------------------------------------------------------------------------------------------------------------- | ------- | ----------------------- | -------------------------------------------------------------------------- |
| E2E-MIG-001 | Migration | Flow A: blob dry-run then apply happy path                                                                                          | covered | `e2e/migration.spec.ts` | hermetic `migration-desktop`/`migration-mobile` projects (scratch `:5011`) |
...
| ID               | Domain       | Flow                                                                                  | Status  | Planned spec file          | Notes                                                                            |
| ---------------- | ------------ | ------------------------------------------------------------------------------------- | ------- | -------------------------- | -------------------------------------------------------------------------------- |
| E2E-ADMINCFG-001 | Admin config | Section matrix: editable T1/T2 db/default keys in Section A; T0 and env-sourced T1/T2 keys read-only in Section B | covered | `e2e/admin-config.spec.ts` | hermetic `admin-config-desktop`/`admin-config-mobile` projects (scratch `:5010`) |
...
| E2E-ADMINCFG-003 | Admin config | Admin warning surface for unreadable DB secrets                                             | removed | `e2e/admin-config.spec.ts` | Row kept for ID stability; deletion with app-layer field encryption (W-A, 2026-09) — DB secret rows are plaintext and always readable, so no loss-warning banner exists to test |
| E2E-ADMINCFG-004 | Admin config | Secret lifecycle: masked on load, `'****'`/blank keeps the stored value, a new value is stored plaintext | covered | `e2e/admin-config.spec.ts` |                                                                                  |
...
| E2E-ADMINCFG-009 | Admin config | T0 keys (incl. `JWT_SECRET`, `WEA_DB_PASSWORD`) render read-only in Section B — set secrets masked `'****'`, unset secrets as `'(unset)'` (presence preserved) — in registry order; supersedes the D5 "metadata group absent" assertion | covered | `e2e/admin-config.spec.ts` | Section split (W-B, 2026-09): T0 keys are shown in the Deploy-time summary, not hidden |
