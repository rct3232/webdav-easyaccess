...
const CreateFolderDialog = ({
  ...
  const { values, isSubmitting, setValue, handleChange, handleSubmit, reset, getFieldError } =
    useFormState(
      { folderName: '' },
      { folderName: validateFileName },
      {
        onSubmit: async (formValues) => {
          ...
          try {
            const folderPath =
              currentPath === '/' ? `/${finalFolderName}` : `${currentPath}/${finalFolderName}`;
            ...
            const errorMsg =
              getServerErrorDisplay(error?.response?.data, t) || t('dialogs.createFolderFail');
