...
| DEF-11 | DONE (2026-09-10)        | Managed version history landed: `object_map.status='history'` (prior versions are managed, not strays), per-node cap `GC_VERSION_MAX_PER_NODE` (default 10, 0=unbounded) with overwrite-TX1 eviction to `orphaned`, `GC_VERSION_TTL_DAYS` = eviction grace, zero-I/O in-place restore, properties-dialog `정보                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | 버전`tabs, attachment-only old-version download, S3 storage mode only. Specs:`blobStorageService.md`, `fileNodesStore.md`, `gcService.md`§2,`routes/files.md`, `FilePropertiesDialog.md`. Migration cutover drops history (documented; DEF-18 class). | `docs/spec/server/services/blobStorageService.md`, `docs/spec/server/store/fileNodesStore.md`, `docs/features/core-service-layer.md` |
| DEF-16 | DONE (2026-09-10)        | Trash/recycle-bin landed across P1–P9: `deleted_at` soft-delete (partial unique indexes over live rows), OS-recycle-bin restore (auto-restores trashed ancestors, suffix collisions), purge (physical, FK-cascade revokes perms at purge time) + admin empty-trash, repo-SQL read-gating everywhere (trashed = 404), permission-based visibility, Tier 3 GC purge (`TRASH_RETENTION_DAYS` 30d, 0=off), `.wea-` reserved namespace + `/.wea-trash/<nodeId>` WebDAV MOVE trash, `__trash__` virtual-root UI with bottom-pinned sidebar entry. Specs: `fileService.md` §4.1, `routes/files.md`, `gcService.md`, `client-ui.md`, `E2E_COVERAGE_PLAN.md` (E2E-TRASH-001..011).                                                                                                                                                                                                                                                                                                                                                                                  | `docs/spec/server/services/fileService.md` §4.1, `docs/spec/server/services/gcService.md` §2, `docs/features/client-ui.md`                                                                                                                            |
...
| DEF-23 | DONE (2026-09-11)        | Decided unneeded; the whole clear-all chain was removed (route, service, store facade, both repo twins, typedef, conformance, `recentFiles.clearedSuccess` code + locales + MSW mock) — `chore/remove-recent-clearall`. Original record below. Recent-files **clear-all** UI: `DELETE /api/recent-files` is a live documented server contract with a working route, but no client caller or feature spec exists (single stale-entry removal is live). Decide whether a clear-all affordance belongs in `RecentFilesSection` before keeping the wrapper-less endpoint forever.                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | `docs/spec/server/routes/recentFiles.md`                                                                                                                                                                                                              |
| DEF-22 | DONE (2026-09-12)        | Logout / refresh-token lifecycle landed in `4ec0a7e` (both recorded options shipped together): `refreshAccessToken` now performs single-use rotation (the presented id is consumed via `tokenStore.deleteRefreshToken` — now wired — and a fresh id is registered; reuse 401s, bounding a stolen token's lifetime to the next legitimate refresh) and `POST /api/auth/logout` revokes one id idempotently (unknown/absent = no-op, no enumeration oracle). Client logout is two-sided (best-effort server revoke + local clear) and persists the rotated token behind a single-flight refresh guard. Server/client tests + `routes/auth.md` shipped with the change; `api.md`, `auth-users-settings.md` endpoint/flow sections and the ARCHITECTURE auth route list synced at tracker closeout. Note: tokens live in the in-memory CacheAdapter, so revocation does not propagate across instances/restarts (pre-existing design property, not a DEF-22 gap).                                                                                              | `server/domains/auth/tokenStore.js`                                                                                                                                                                                                                   |
...
| DEF-21 | DONE (2026-09-11)        | The batch worker dispatched `batchMove`/`batchCopy` with a synthesized `{ is_admin: true }` (introduced in `9b38acb` with no stated rationale — archaeology confirmed it contradicts the W3.4 spec, the pre-existing `getCachedUser` mechanism, and even the delete case in the same switch; the batch routes never had any ACL pre-check, so every authenticated user had full admin semantics on move/copy through the canonical channel). Fixed: `_processBulkJob` resolves the requesting principal via `aclService.getCachedUser(job.userId)` (missing user → job `failed` with `auth.userNotFound`) and passes the REAL user to all three dispatches; per-item ACL + the D6 ownership-transfer cleanup now behave exactly as direct non-admin calls. Regression coverage: cross-user batch-move/-copy skipped `permission_denied`; C2/D6 scenarios moved back onto the batch HTTP channel. Note: the original DEF-21 wording overstated two points — the delete case never used the synthesized admin, and no route-level destination check existed. |
...
- **Trigger**: introducing the NEXT engine from the DEF-14 target list (MySQL / MariaDB /
  ...
  repository _implementations_ only, and per-engine conformance suites remain mandatory.
...
### Completion note (2026-09-13) — DEF-22 tracker closeout

- DEF-22 (logout / refresh-token lifecycle) shipped on 2026-09-12 in `4ec0a7e` (single-use
  refresh rotation + idempotent `POST /api/auth/logout`, `deleteRefreshToken` wired, two-sided
  client logout) but the tracker row was never closed. This closeout marks it DONE and syncs the
  remaining stale docs: `api.md` auth table (logout row + rotation), `auth-users-settings.md`
  endpoint table and Token refresh/Logout flow sections, `ARCHITECTURE.md` auth route list, and
  the client MSW refresh mock (now returns a rotated `refreshToken`, mirroring the real contract).
...
- Hermetic overlap (W5 + Option A Phases 1–2): mypage-admin moved to dedicated post-reset
  ...
  scratch port (:5003 wizard / :5010 admin-config / :5011 migration / :5012 trash-admin) mirrored by its
