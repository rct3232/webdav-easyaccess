...
| DELETE | `/api/files/delete`                       | Token          | Delete (trash) one node. Body: `{ nodeId }`. Marks the whole subtree `deleted_at` (soft delete); WebDAV mode moves the remote subtree once to `/.wea-trash/<nodeId>`. Returns `{ deletedCount }`.                                                                                                                                                                          |
