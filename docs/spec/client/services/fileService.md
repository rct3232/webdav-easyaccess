...
| getFileVersions          | (nodeId)                                                         | Promise\<Object\>                                     | GET /api/files/versions (params: nodeId). Returns `{ nodeId, currentVersionNumber, versions: [{ versionNumber, status, createdAt, size, isCurrent }] }`.                                                                                             |
| restoreFileVersion       | (nodeId, versionNumber)                                          | Promise\<Object\>                                     | POST /api/files/versions/restore `{ nodeId, versionNumber }` — returns `{ messageCode, nodeId, restoredVersionNumber }`.                                                                                                                             |
| downloadFileVersion      | (nodeId, versionNumber)                                          | Promise\<void\>                                       | GET /api/files/versions/download (params: nodeId, versionNumber) as blob + `<a download>` trigger (attachment-only endpoint).                                                                                                                        |
| getTrashFiles            | (options?)                                                       | Promise\<Object\>                                     | GET /api/files/trash. Returns `{ items: [{ nodeId, name, type, deletedAt, displayPath, hasReadPermission, hasWritePermission, hasAdminPermission }], total }` — the caller-visible trashed rows (DEF-16; consumed by the trash view, full UI in P9). |
...
- [ ] getFileVersions requests GET /api/files/versions?nodeId= and returns the parsed payload
- [ ] restoreFileVersion posts `{ nodeId, versionNumber }` to POST /api/files/versions/restore
- [ ] downloadFileVersion fetches the version blob and triggers an `<a download>` save (no iOS share path — the endpoint is attachment-only; used from the properties dialog's versions tab)
- [ ] getTrashFiles requests GET /api/files/trash and returns the parsed `{ items, total }` payload
