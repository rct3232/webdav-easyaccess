...
| Dependencies | `migrationService` (`getMigrationStatus`, `getBlobMigrationStatus`, `cancelBlobMigration`, `ackLastMigrationJob`), MUI, `react-i18next`, `formatDate`                                                                                                                                                                         |

## 2. Behavior

- **Mount resolve:** one-shot `getMigrationStatus()` on mount. If `active` with a `jobId`, render the running/terminal job view (poll below). If `!active` but the admin response carries `lastJob` (a finished migration the page missed), load that job via `getBlobMigrationStatus(lastJob.jobId)`, render its terminal view, and call `ackLastMigrationJob()` once the fetch settles — the completion notice then survives no further mount, keeping the modal exactly-once even when the job finished before mount.
- **Polling:** `getBlobMigrationStatus(jobId)` every 400 ms while the job is non-terminal (`completed` / `failed` / `cancelled` stop it). A recovered `lastJob` is already terminal, so no polling starts for it.
