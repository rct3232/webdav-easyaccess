...
describe('createMigrationService', () => {
  ...
  it('webdav→s3: flat UUID keys only, object_map flipped to s3, hashes match, filecache hash set', async () => {
    ...
    const f2 = await seedWebdavFile({ parentId: docs.nodeId, name: 'b.txt', content: 'beta-beta', srcStore: src });
    const f3 = await seedWebdavFile({ parentId: docs.nodeId, name: 'c.txt', content: 'gamma', srcStore: src });
  ...
  it('s3→webdav: dest paths preserve the tree, ancestor dirs recorded, object_map flipped to webdav', async () => {
    ...
    const f2 = await seedS3File({ parentId: docs.nodeId, name: 'b.txt', content: 'beta', srcStore: src });
  ...
  it('dry-run writes nothing, leaves DB untouched, returns dryRun:true', async () => {
    ...
    const src = createFakeBlobStore();
    const f1 = await seedWebdavFile({ parentId: rootNodeId, name: 'a.txt', content: 'alpha', srcStore: src });
    const f2 = await seedWebdavFile({ parentId: rootNodeId, name: 'b.txt', content: 'beta', srcStore: src });

    const dst = createFakeBlobStore();
    buildDestBlobStore = () => ({ blobStore: dst, summary: 's3 fake' });
    const service = makeService(src);

    const result = await service.run({ direction: 'webdav-to-s3', destConfig: { type: 's3' }, mode: 'dry-run' });

    expect(result.dryRun).toBe(true);
    expect(result.copied).toBe(0);
    expect(dst.count()).toBe(0);

    const expectedHashes = { [f1.nodeId]: sha256HexLower('alpha'), [f2.nodeId]: sha256HexLower('beta') };
    for (const file of [f1, f2]) {
      const row = await getActiveObjectRow(file.nodeId);
      expect(row.storage_backend).toBe('webdav');
      expect(row.s3_key).toBeNull();
      const cache = await dbQuery('SELECT content_hash FROM filecache WHERE file_node_id = ?', [file.nodeId]);
  ...
  it('webdav source rejects a webdav destination before any work', async () => {
    ...
    await expect(
      service.run({ destConfig: { type: 'webdav', url: 'http://dav', username: 'u', password: 'p' }, mode: 'apply' })
  ...
  it('s3 source rejects an s3 destination before any work', async () => {
    ...
    await expect(
      service.run({ destConfig: { type: 's3', bucket: 'b', accessKey: 'ak', secretKey: 'sk' }, mode: 'apply' })
  ...
  it('apply aborts with no writes when the internal dry pass (dest probe) fails', async () => {
    ...
    await expect(
      service.run({ destConfig: { type: 's3' }, mode: 'apply' })
    ).rejects.toThrow('destination unreachable');
  ...
  it('exclusive lock: concurrent run is rejected with "migration already in progress"', async () => {
    ...
    const held = await lockManager.acquireLock('migration:blobs', { ttlMs: 60 * 1000, waitMs: 1000 });
    try {
      await expect(
        service.run({ destConfig: { type: 's3' }, mode: 'apply' })
