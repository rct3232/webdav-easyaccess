...
router.post('/file/grant', authenticateToken, requireUser, asyncHandler(async (req, res) => {
...
router.delete('/file/revoke', authenticateToken, requireUser, asyncHandler(async (req, res) => {
...
router.patch('/file', authenticateToken, requireUser, asyncHandler(async (req, res) => {
  const { userId, fileNodeId, permission } = req.body;
...
router.get('/file/check', authenticateToken, requireUser, asyncHandler(async (req, res) => {
  const fileNodeId = req.query.fileNodeId;
  ...
  const hasRead = effectivePerm ? PERMISSIONS.isValid(effectivePerm) && (effectivePerm === PERMISSIONS.READ || effectivePerm === PERMISSIONS.WRITE || effectivePerm === PERMISSIONS.ADMIN) : false;
  const hasWrite = effectivePerm ? (effectivePerm === PERMISSIONS.WRITE || effectivePerm === PERMISSIONS.ADMIN) : false;
...
router.get('/file/list', authenticateToken, requireUser, asyncHandler(async (req, res) => {
  const userId = req.user.id;
