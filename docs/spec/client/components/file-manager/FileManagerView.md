...
| trashState          | object | N        | Trash-view overlay group (DEF-16 P9): `trashMode`, breadcrumb `trail`, `canEmptyTrash`, the three trash confirm states + open/close/confirm callbacks (`useTrashOperations`), and the `handleTrashRestore`/`handleTrashPurge` executors. The view keys trash behavior off `trashMode`/`currentPath === '/__trash__'` (breadcrumb trail, controls trash buttons, withheld live-item menu/sheet callbacks, trash confirm dialogs, trash `emptyText`). |
