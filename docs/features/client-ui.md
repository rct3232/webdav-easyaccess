...
- **Single-file download (iOS + image):** On iOS (e.g. iPhone Chrome), downloading an image file uses a photo-save–friendly path so the user can save to Photos instead of only to Files/Chrome. **Policy:** (1) If the browser supports sharing files (`navigator.canShare` with a `File`), the app uses the Web Share API so the system share sheet appears; the user chooses "Save Image" (or equivalent) to save to Photos. (2) If Web Share with files is unavailable or fails, the app falls back to opening the image inline (e.g. blob URL in a new tab) so the user can long-press and save from the browser. (3) All other cases (desktop, non-image, folder download, multi-file zip) keep the existing blob + `<a download>` behavior. **User guidance:** When the share sheet is shown, the UI may show a short hint (e.g. tooltip or toast) that the user can select "Save Image" to save to Photos. See `docs/spec/client/services/fileService.md` (§ 2.3) for the service-level spec.
