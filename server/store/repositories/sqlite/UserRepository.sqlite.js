...
/**
 * sqlite implementation of UserRepository (`?` placeholders, executor-driven).
 * @param {import('../../../infrastructure/db/executor').DbExecutor} executor
 */
module.exports = function createSqliteUserRepository(executor) {
  return {
    dialect: 'sqlite',
    ...
    async createUser({ username, email, passwordHash, isAdmin = false }) {
      ...
      try {
        return await executor.transaction(async (tx) => {
          const dupUsername = await tx.query('SELECT 1 FROM users WHERE username = ? LIMIT 1', [
            String(username),
          ]);
          if (dupUsername.rows.length > 0) throwUsernameTaken();

          const dupEmail = await tx.query('SELECT 1 FROM users WHERE email_hash = ? LIMIT 1', [
            emailHash,
          ]);
          ...
          const inserted = await tx.query('SELECT * FROM users WHERE id = last_insert_rowid() LIMIT 1');
    ...
    async updateEmail(userId, newEmail) {
      ...
      try {
        return await executor.transaction(async (tx) => {
          const currentUserRes = await tx.query('SELECT * FROM users WHERE id = ? LIMIT 1', [
            userIdNum,
          ]);
          if (currentUserRes.rows.length === 0) throwUserNotFound();

          const dupEmailRes = await tx.query(
            'SELECT id FROM users WHERE email_hash = ? LIMIT 1',
            [newHash]
          );
    ...
    async deleteUser(userId) {
      try {
        await executor.transaction(async (tx) => {
          await tx.run('DELETE FROM users WHERE id = ?', [Number(userId)]);
        });
        return { success: true };
      } catch (error) {
        throw mapDatabaseError(error);
