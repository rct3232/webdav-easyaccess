...
| POST   | `/api/files/upload`                       | Token          | Upload file. Multipart: `file`; form fields: `parentNodeId`, `onConflict` (error, overwrite, skip), optional `relativePath`. Reserved-name rejection: a file name starting with `.wea-` (case-insensitive) is rejected 400 `files.fileNameReserved` (reserved server namespace).                                                                                           |
| PUT    | `/api/files/rename`                       | Token          | Rename. Body: `{ nodeId, newName }`. Reserved-name rejection: a `.wea-`-prefixed (case-insensitive) `newName` is rejected 400 `files.fileNameReserved`.                                                                                                                                                                                                                    |
| GET    | `/api/files/trash`                        | Token          | List trashed nodes visible to the caller (write-permission survivors + admin). Query: `parentId` (optional — navigate a trashed folder's trashed children; omitted = topmost trashed items only), `limit` (default 50), `offset`. Returns `{ items: [{ nodeId, name, type, size, deletedAt, displayPath, hasReadPermission, hasWritePermission, hasAdminPermission }], total }` (`size` filecache-backed: real bytes for files, null for directories/cache-less rows). |
| POST   | `/api/files/trash/restore`                | Token          | Restore one trashed node to its original location. Body: `{ nodeId }`. Write perm on the node + live parent; auto-restores trashed ancestors (siblings stay trashed); name collisions auto-suffix `name (2).ext`. Returns `{ messageCode, nodeId, restoredNodes, finalPath }`. 409 `files.notTrashed` for a live node.                                                     |
| POST   | `/api/files/trash/purge`                  | Token          | Permanently delete ONE trashed node (physical delete in both storage modes; FK cascade removes permission/share rows). Body: `{ nodeId }`. Delete perm (write, admin bypasses); 409 `files.notTrashed` for a live node. Returns `{ messageCode, nodeId, purgedNodes, deletedBlobs }`.                                                                                      |
| POST   | `/api/files/trash/empty`                  | Token + Admin  | Permanently delete ALL topmost trashed nodes (best-effort). Returns `{ purgedNodes, purgedBlobs, errors: [] }`. 403 for non-admin.                                                                                                                                                                                                                                         |
| POST   | `/api/files/batch-move`                   | Token          | Move items (canonical mutation channel — also for a single node). Body: `{ moves: [{ sourceNodeId, destinationParentNodeId }] }`, optional `onConflict` (error, overwrite, skip). Returns 202 + `jobId`; poll `GET /api/files/bulk-operation/:jobId` until terminal.                                                                                                                                                                                                                         |
| POST   | `/api/files/batch-copy`                   | Token          | Copy items (canonical mutation channel — also for a single node). Body: `{ copies: [{ sourceNodeId, destinationParentNodeId, newName }] }`, optional `onConflict`. Returns 202 + `jobId`; poll `GET /api/files/bulk-operation/:jobId` until terminal.                                                                                                                                                                                                                                        |
| POST   | `/api/files/batch-delete`                 | Token          | Delete items (canonical mutation channel — also for a single node). Body: `{ nodeIds }`. Returns 202 + `jobId`; poll `GET /api/files/bulk-operation/:jobId` until terminal. Each item TRASHES the subtree (soft delete `deleted_at`; WebDAV moves the remote subtree once to `/.wea-trash/<nodeId>`).                                                                                                                                                                                                                                                                                                                  |
...
| POST   | `/api/files/metadata`                     | Token or share | Get metadata for nodeIds. Body: `{ nodeIds }`. Returns `[{ nodeId, name, type, size, lastmod, mime }]` — `size`/`mime` from `filecache` (null for directories/cache-less rows); unreadable nodes are skipped.                                                                                          |
...
| GET    | `/api/files/versions`                     | Token          | Browse version history (S3 storage mode only). Query: `nodeId`. Returns `{ nodeId, currentVersionNumber, versions: [{ versionNumber, status, createdAt, size, isCurrent }] }`; storage internals stripped. No read permission → 404; share token → 403.                                                                                                                    |
| POST   | `/api/files/versions/restore`             | Token          | Restore one version in place (S3 storage mode only). Body: `{ nodeId, versionNumber }`. Write perm; 404 `files.versionNotFound` for unknown version; 409 when WebDAV mode, node stuck `pending_upload` (`files.versionRestoreUnavailable`) or the version's blob is missing (`files.versionBlobMissing`).                                                                  |
| GET    | `/api/files/versions/download`            | Token          | Download one version attachment-only (`application/octet-stream`). Query: `nodeId`, `versionNumber`. Read perm (404-masquerade); share token → 403.                                                                                                                                                                                                                        |
| POST   | `/api/folders/create`                     | Token          | Create folder. Body: `{ parentNodeId, name }`. Reserved-name rejection: a `.wea-`-prefixed (case-insensitive) `name` is rejected 400 `files.fileNameReserved`.                                                                                                                                                                                                             |
...
| GET    | `/api/admin/config/sync-report`   | Token + Admin | Env↔DB drift report for the running process env; same `findings`/`summary`/`exitCode` JSON as the config-sync CLI `--check`.                                                                                                                                          |
| POST   | `/api/admin/config/sync-from-env` | Token + Admin | Mirror env-set non-T0 config values into DB `settings` (web equivalent of the config-sync CLI `--apply --yes`). Returns `{ writes, report, messageCode }`.                                                                                                            |
