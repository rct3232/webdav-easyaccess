...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const filteredPermissions = await Promise.all(
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!canView && fileNodeId) {
      ...
      const hasRead = await aclCheckFilePermission(
        req.user.id,
        Number(fileNodeId),
        PERMISSIONS.READ
      );
    ...
    if (includeDescendants) {
      ...
      const allPerms = [];
      for (const descId of descendantIds) {
        const perms = await permissionStore.getFolderPermissions(
          descId,
      ...
      const parentPerms = await permissionStore.getFolderPermissions(
        nodeId,
        fileNodeId ? Number(fileNodeId) : undefined
      );
