...
|------|------|----------|-------------|
| path | string | N | Folder path (used when `node` is not provided) |
| name | string | N | Folder display name (used when `node` is not provided) |
| node | object | N | Node { path, name, isHidden, hasReadPermission, hasWritePermission, children } |
| level | number | N | Indent level (kept for API completeness; controller may not use) |
| currentPath | string | Y | Current explorer path for auto-expansion |
| expandedPaths | Set | Y | Expanded paths set |
| onToggleExpand | function | Y | Toggle expanded state for a path |
| onPathClick | function | Y | Folder click callback (called when enabled) |
...
| onInternalFileDrop | function | N | Internal drag/drop: (draggedPath, targetFolderPath) |
| onInternalDragStart | function | N | Called when drag starts: (path) => void |
| onInternalDragEnd | function | N | Called when drag ends: () => void |
| internalDraggedPath | string | N | Last internal drag source path for drop no-op logic |
...
| sharedFoldersMap | Map | N | Shared-folder permission map that can override permissions |
...
| Key | Type | Meaning |
|-----|------|---------|
| path | string | Resolved folder path |
...
| isExpanded | boolean | Whether `expandedPaths` contains `path` |
| isCurrent | boolean | Whether `currentPath === path` |
...
| handleClick | function | Folder click handler (respects disabled) |
| handleToggle | function | Expand/collapse handler (and initial load) |
| handleDragStart | function | Drag start handler for tree drag source |
| handleDragEnd | function | Drag end handler for tree drag source |

### 2.4 Dependencies

- Services called:
  - `client/src/services/folderTreeGateway.listFolderChildren`
- Other hooks:
  - `client/src/hooks/useDropToUpload`
- Ownership note:
...
- Lazy-loads children via `folderTreeGateway.listFolderChildren` when:
  - the item is expanded (`expandedPaths.has(path)`),
  - children have not been loaded (`!hasLoaded`),
  - and no load is currently running (`!loading`).
- Auto-expands ancestors when `currentPath` is under the item path.
- Reconciles children when `treeUpdateTrigger` changes:
  - `created`: inserts a new child under `parentPath === path` and expands if needed
  - `deleted`: removes a child by `folderPath`
...
- [ ] When expanded and never loaded, lazy loading calls `folderTreeGateway.listFolderChildren` and renders children in `BaseFolderTreeItem`.
- [ ] `treeUpdateTrigger.type === 'created'` adds a missing child (deduped by `child.path`) and expands when needed.
- [ ] `treeUpdateTrigger.type === 'deleted'` removes the matching child.
...
- [ ] Drag start publishes the dragged path only when the item is enabled and not mobile; drag end notifies the host cleanup callback.

### 2.8 Edge Cases

- `path/name` can be provided either directly or via `node`.
- `sharedFoldersMap` can override permission derivation when `node` does not carry permission fields.
