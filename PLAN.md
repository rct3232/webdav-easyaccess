...
- `server/domains/admin/services/metadataMigrationService.js` (new): direct target connection (`pg.Client` / `sqlite3.Database`, `probePostgresql` pattern); target scan (`schemaExists`, per-table row counts); schema apply to explicit target (D6 refactor); wipe + copy (FK order: users → file*nodes → object_map/filecache/node_ancestors → permissions*\* → share_links/recent_files/permission_requests → locks → settings) with settings JSON-string serialization, explicit ids + `setval`/`sqlite_sequence` resync; single target transaction; cancel = rollback.
...
- 2026-09-01 (branch `fix/webdav-migration-source`): M1 docs-first for the A2 defect (decision now made). Documented fix design: (1) `enumerateSnapshot` source-mode aware — webdav source enumerates non-orphaned file nodes via new `getNodesBySyncStatusNot('orphaned_node')` and per node uses the active `object_map` (preserved `s3_key` resume marker) else synthesizes `{ s3_key: null, storage_backend: 'webdav' }`; S3 mode unchanged; (2) new `fileNodesStore.getNodesBySyncStatusNot(status)` (PG + sqlite); (3) webdav→s3 `processNode` success sets `sync_status='active'`; (4) `gcService.runTier1` in webdav mode skips `blobStore.deleteBlob` (preserved UUID `s3_key` is not a webdav path) while still deleting `object_map` rows; Tier 2 already skipped (gcService.js:93). E2E correction scope: drop the `seedObjectMapRows` snapshot precondition for webdav→s3, seed real webdav-native uploads, assert `copied>0` + post-copy `sync_status='active'`. Docs updated: fail_log RCA entry, migrationService / fileNodesStore / gcService / blob-migration specs, migration-mode feature, PLAN log. Code implementation follows.
