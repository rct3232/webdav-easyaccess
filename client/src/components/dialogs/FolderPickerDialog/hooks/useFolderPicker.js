...
export function useFolderPicker({
  ...
  const [selectedNodeId, setSelectedNodeId] = useState(
    currentNodeId != null ? currentNodeId : homeNodeId
  );
  const [navStack, setNavStack] = useState(() => [{ nodeId: homeNodeId ?? null, name: homeLabel }]);
  ...
  const loadFolders = useCallback(
    async (nodeId, pathType = getCurrentPathType()) => {
      setLoading(true);
      try {
        if (pathType === 'shared' && nodeId == null) {
          const sharedPermissions = await folderPickerGateway.getUserSharedFolderPermissions({
            user,
          });
