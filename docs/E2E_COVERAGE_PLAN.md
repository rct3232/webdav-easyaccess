...
| E2E-OVERLAY-008 | Virtual collection | Approved user can enter `__recent__` and see recent entries | approved user | `/files` | both | recent entries exist | browser navigates into the recent collection and the entries render with recent metadata | P0 | playwright | `e2e/explorer-advanced.desktop.spec.ts`, `e2e/explorer-advanced.mobile.spec.ts` | covered | Keep deeper recent-sync rules in lower layers |
